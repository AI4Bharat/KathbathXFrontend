...
import com.ai4bharat.kathbath.data.model.karya.ChecksumAlgorithm
import com.ai4bharat.kathbath.data.model.karya.MicroTaskAssignmentRecord
import com.ai4bharat.kathbath.data.remote.request.UploadFileRequest
...
import com.ai4bharat.kathbath.utils.extensions.getBlobPath
...
class DashboardSyncWorker(
    ...
    private suspend fun syncWithServer() {
        ...
        println("Sync with server called")
        try {
            val worker = authManager.getLoggedInWorker()
            assignmentRepository.updateExpired(worker.id)
            println("Sync ${worker.fullName}")
        } catch (e: Exception) {
            println("Syncing error in getting worker")
...
//            return
        ...
        try {
            ...
            println("Syncing error 2 ${e.toString()}")
        ...
        try {
            ...
            println("Syncing error 3 ${e.toString()}")
            FirebaseCrashlytics.getInstance().recordException(e)
        ...
        try {
            ...
            println("Syncing error 4 ${e.toString()}")
            FirebaseCrashlytics.getInstance().recordException(e)
        ...
        try {
            ...
            println("Syncing error 5 ${e.toString()}")
            FirebaseCrashlytics.getInstance().recordException(e)
