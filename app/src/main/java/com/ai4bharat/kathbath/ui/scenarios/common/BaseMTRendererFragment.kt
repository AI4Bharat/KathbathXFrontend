package com.ai4bharat.kathbath.ui.scenarios.common
...
abstract class BaseMTRendererFragment(@LayoutRes contentLayoutId: Int) :
    ...
    private fun setUpObservers() {
        viewModel.completedAssignments.observe(
        ...
        viewModel.inputFileDoesNotExist.observe(
            viewLifecycleOwner.lifecycle,
            lifecycleScope
        ) { notExist ->
            if (notExist) {
        ...
        viewModel.outsideTimeBound.observe(
            viewLifecycleOwner.lifecycle,
            lifecycleScope
        ) { outside ->
            if (outside.first) {
                val builder = AlertDialog.Builder(requireContext())
                val startTime = DateTimeUtils.convert24to12(outside.second)
                val endTime = DateTimeUtils.convert24to12(outside.third)
