...
import com.ai4bharat.kathbath.utils.jtar.TarEntry
import com.ai4bharat.kathbath.utils.jtar.TarInputStream
import com.ai4bharat.kathbath.utils.jtar.TarOutputStream
...
object FileUtils {
    ...
    fun splitFileName(fileName: String): Pair<String, String> {
        val split = fileName.replace("\"", "").split(".")
        return Pair(split[0], split[1])
    }

    fun copyFile(sourceFullPath: String?, destinationFullPath: String?) {
        if (sourceFullPath != null && destinationFullPath != null) {
            val source = FileInputStream(sourceFullPath)
            val destination = FileOutputStream(destinationFullPath)
