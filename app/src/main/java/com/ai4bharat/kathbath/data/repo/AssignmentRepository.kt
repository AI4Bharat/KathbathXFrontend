...
import com.ai4bharat.kathbath.data.local.daos.MicroTaskAssignmentDao
import com.ai4bharat.kathbath.data.local.daos.MicroTaskDao
import com.ai4bharat.kathbath.data.local.daos.TaskDao
...
import com.ai4bharat.kathbath.data.service.MicroTaskAssignmentAPI
...
constructor(
    ...
    fun submitCompletedAssignments(idToken: String, updates: List<MicroTaskAssignmentRecord>) =
        flow {
            if (idToken.isEmpty()) {
                error("Either Access Code or ID Token is required")
            }
    ...
    // Added by TJ
    suspend fun getTotalRecordedTasks(workerId: String): Pair<Float, Float> {
        var read = 0.0f
        var extempore = 0.0f
        for (taskRecord in taskDao.getAll()) {
...
//      }
            ...
            if (taskRecord.name.lowercase().contains("[extempore]") || taskRecord.name.lowercase()
                    .contains("[semi-extempore]")
            ) {//&& (t.name.lowercase().contains("kyp") || t.name.lowercase().contains("toi"))) {
                var dur: Float = 0.0F
                for (i in assignmentDaoExtra.getTotalRecordedTasks(workerId, taskRecord.id)) {
                    val jp = JsonParser.parseString(i).asJsonObject
                    dur += jp.get("data").asJsonObject.get("duration").asFloat
                }
                extempore += dur
            } else if (taskRecord.name.lowercase()
                    .contains("[read]")
            ) {//(t.name.lowercase().contains("wikipedia") || t.name.lowercase().contains("alexa") || t.name.lowercase().contains("bigbasket") || t.name.lowercase().contains("digital") || t.name.lowercase().contains("umang")){
                var dur: Float = 0.0F
                for (i in assignmentDaoExtra.getTotalRecordedTasks(workerId, taskRecord.id)) {
                    val jp = JsonParser.parseString(i).asJsonObject
                    dur += jp.get("data").asJsonObject.get("duration").asFloat
                ...
                // extempore task
                    ...
                    .contains("task of fives")
            ) {//(t.name.lowercase().contains("wikipedia") || t.name.lowercase().contains("alexa") || t.name.lowercase().contains("bigbasket") || t.name.lowercase().contains("digital") || t.name.lowercase().contains("umang")){
                if (assignmentDaoExtra.getCountForTask(
                        taskRecord.id,
                        MicrotaskAssignmentStatus.SKIPPED
                    ) + assignmentDaoExtra.getCountForTask(
                        taskRecord.id,
